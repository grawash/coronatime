<x-user-layout>
    <form method="POST" action="{{ route('login') }}" class="mt-16">
        ...
        <p class="text-neutral-600 text-xl mt-4">{{__('login.welcome_text') }}</p>
        ...
        <div class="flex mt-6 items-center">
            ...
            <label for="remember" class="pl-2 font-semibold text-sm">{{__('login.remember') }}</label>
            <a href="{{ route('password.request') }}" class="ml-auto text-blue-700 font-semibold text-sm">{{__('login.forgot_password') }}</a>
        </div>
